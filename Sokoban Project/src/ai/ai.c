...
void free_memory(unsigned expanded_nodes, sokoban_t* init_data){
	for( unsigned i = 0; i < expanded_nodes; i++){
		free_node(expanded_nodes_table[ i ], init_data);
...
void find_solution(sokoban_t* init_data, bool show_solution)
{
	...
	while (pq.count != 0){
		...
		for(int a = left; a <= down; a++){
			node_t* newNode;
			bool playerMoved = applyAction(init_data, n, &newNode, a);
			generated_nodes++;

			if(playerMoved == false || simple_corner_deadlock(init_data, &(newNode->state))){
				free_node(newNode, init_data);
				free(newNode);
				continue;
			}
...
void free_heap(struct heap* h, sokoban_t *init_data) {
    ...
    // Free each node_t* in the heaparr
    for (int i = 0; i < h->count; i++) {
        if(h->heaparr[i] != NULL){
			free_node(h->heaparr[i], init_data);
			free(h->heaparr[i]);
		}
    }

    // Free the heaparr array
    free(h->heaparr);
}
...
void free_node(node_t *n, sokoban_t *init_data){
    ...
    // Free the map array within the state
    for (int i = 0; i < init_data->lines; i++){
        free(n->state.map[i]);
    }
    free(n->state.map);
